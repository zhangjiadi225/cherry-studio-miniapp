...
- Treat this directory as the only Git repository. `foundation/`, every `apps/*/` directory, and every `examples/*/` directory are workspace areas inside the monorepo; do not create nested Git repositories or configure per-directory remotes.
...
- Put runnable capability demonstrations that are not released products in `examples/<slug>/`; examples must not be registered as products or included in batch app builds and packaging.
