...
## Workflow
1. Edit the relevant `renderers/*` (or add an `effects/*` asset → draw fn in `EffectsRenderer` → call from `Game.ts`). 2. `pnpm dev` and **look at it** — visual work must be eyeballed, not just typechecked. 3. `pnpm build` to keep `tsc` green. Adding new entity art? See the `web-game-dev` skill's render-branch steps. New colors belong in `data/colors.ts`, not inline.
