...
| `input` | 键盘与触控摇杆 |
| `camera` | 跟随、固定近景缩放与震动；缩放只影响画面和可见查询，不改变玩法坐标 |
...
- Enemy、Projectile、EnemyProjectile、Particle 和 DamageNumber 池按对应运行时硬上限预热；性能面板记录战斗期间的池耗尽创建次数。
...
- 空间查询支持命中额度耗尽后的提前终止；地图圆形碰撞、弹体命中上下文、粒子上下文和常用查询回调复用稳定对象，避免在固定步热路径持续制造临时对象。
- Map 空间查询和可见缓存。
- 敌人、玩家弹幕、敌方弹幕、粒子和飘字硬上限。
- Canvas 实际渲染倍率最高为 1.5；粒子辉光使用有限缓存贴图，屏幕内粒子按形状、辉光和光束复杂度计算渲染成本并在超预算时只降低视觉质量。
- 每个 RAF 帧最多准入 96 个新视觉粒子，避免固定步追赶把多步粒子一次性堆入同一渲染帧；该限制不改变伤害、碰撞和规则随机序列。
- 性能面板可通过启动参数 `?perf`、已保存设置或运行时按 `F3` 临时切换；除总 Update/Render 外，还记录移动、敌人、武器、战斗和表现阶段耗时、粒子绘制耗时与质量、发射拒绝数、空间查询候选/精确检测/提前终止、对象池耗尽次数、固定步追赶数、丢弃时间、规则种子及各硬上限饱和帧数。
- 批量压缩/释放死亡实体。
