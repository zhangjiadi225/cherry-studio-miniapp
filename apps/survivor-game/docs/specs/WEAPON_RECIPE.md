...
- 当前单武器上限为每秒 320 个粒子、理论同时在场 480 个粒子，运行时仍受全局 800 粒子硬上限和每个 RAF 帧 96 个新粒子准入上限保护。后者只裁剪纯视觉实体，避免固定步追赶造成同帧爆发，不得改变伤害、碰撞或规则随机序列。
- 渲染器按形状、辉光和光束复杂度计算屏幕内粒子成本；超过全屏成本预算时可以关闭辉光、减少光束/弧线层数，但不得删除玩法实体或改变粒子之外的规则结果。
...
运行时硬上限触发时，采用确定性准入策略：按施放序号和子弹索引从小到大生成，容量或同帧视觉准入额度耗尽后拒绝后续生成，并记录可聚合的诊断计数。不能随机丢弃或突破对象池上限。
